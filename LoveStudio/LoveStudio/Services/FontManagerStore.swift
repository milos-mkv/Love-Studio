...
struct FontManagerStore {

    static let folder = ".love-studio/fonts"

    @discardableResult
    static func save(_ config: FontManagerConfig, to projectRoot: URL) throws -> URL {
    ...
    static func loadAll(from projectRoot: URL) -> [FontManagerConfig] {
        let dir = projectRoot.appendingPathComponent(folder)
        ...
        return urls
            ...
            .compactMap { url in
                guard let data = try? Data(contentsOf: url),
                      let cfg  = try? JSONDecoder().decode(FontManagerConfig.self, from: data)
                else { return nil }
                return cfg
            }
    }

    static func delete(_ config: FontManagerConfig, from projectRoot: URL) {
    ...
    static func safeName(_ name: String) -> String {
        ...
        return safe.isEmpty ? "Fonts" : safe
    }
}
