...
struct CameraStore {
    ...
    static let folder = ".love-studio/camera"

    @discardableResult
    static func save(_ config: CameraConfig, to projectRoot: URL) throws -> URL {
    ...
    static func loadAll(from projectRoot: URL) -> [CameraConfig] {
        let dir = projectRoot.appendingPathComponent(folder)
        ...
        return urls
            ...
            .compactMap { url in
                guard let data = try? Data(contentsOf: url),
                      let cfg  = try? JSONDecoder().decode(CameraConfig.self, from: data)
                else { return nil }
                return cfg
            }
    }

    static func delete(_ config: CameraConfig, from projectRoot: URL) {
